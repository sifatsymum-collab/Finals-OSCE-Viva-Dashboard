...
const osceEnhancementsById = {
  "cardiac-device-sternotomy": {
    ...
    investigationsPrepend: [
      "Examiner expects ECG first, then CXR / echo / device check depending on the clue you found.",
    ],
    vivaAppend: [
  ...
  "renal-transplant-dialysis": {
    ...
    investigationsPrepend: [
      "Examiner expects renal profile, FBC, urinalysis, and transplant-specific tests if graft dysfunction is suspected.",
    ],
  },
  "ibd-stoma": {
    signsPrepend: [
  ...
  "microcytic-anaemia-sob": {
    investigationsPrepend: [
      "Examiner expects you to prove iron deficiency, grade severity, and find the source rather than just prescribe iron.",
    ],
    vivaAppend: [
...
function refineInvestigations(condition) {
  const intro =
    osceEnhancementsById[condition.id]?.investigationsPrepend ?? [
      "Examiner expects you to confirm the diagnosis, assess severity, and look for cause or complications.",
    ];

  return uniqueItems([...intro, ...condition.investigations]);
}

function refineManagementSection(items, label) {
  const intros = {
    initial: "Immediate viva answer: stabilise if unwell, then treat the obvious problem.",
    definitive:
      "Definitive management: treat the underlying lesion or disease process, not just the sign.",
    longTerm:
      "Long-term: follow-up, complication prevention, and functional optimisation are often exam favourites.",
    mdt: "Who to involve: name the specialty team and the key allied professionals.",
  };

  return uniqueItems([intros[label], ...items]);
}
...
const conditions = rawConditions.map((condition) => ({
  ...
  management: {
    initial: refineManagementSection(condition.management.initial, "initial"),
    definitive: refineManagementSection(condition.management.definitive, "definitive"),
    longTerm: refineManagementSection(condition.management.longTerm, "longTerm"),
    mdt: refineManagementSection(condition.management.mdt, "mdt"),
  },
...
function getTabSections(condition, tabId) {
  switch (tabId) {
    ...
    case "investigations":
      return [{ heading: "Investigations", items: condition.investigations }];
    case "management":
      return [
        { heading: "Initial", items: condition.management.initial },
        { heading: "Definitive", items: condition.management.definitive },
        { heading: "Long-term", items: condition.management.longTerm },
        { heading: "MDT / referrals / who to involve", items: condition.management.mdt },
      ];
    case "prognosis":
